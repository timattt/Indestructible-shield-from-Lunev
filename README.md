...
Есть - системные вызовы чтения и записи во время работы с transfer fifo, конфликт за память. Вторая критическая секция начинается при входе в ожидательный цикл, а выход в тот момент, когда кто-то умирает, конфликтуют читатель и писатель, конфликт за второй фифо.
...
* ридеры друг с другом в самом начале когда выбирается только один из них (семоп1 и до смерти).
