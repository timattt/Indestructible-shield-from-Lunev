...
Это происходит потому что для определения прав доступа используется константа 666, а не 0666. Исходя из того, что права для fifo в системном вызове mkfifo задаются выражением (mode | umask), где mode - параметр. Получается, что если umask заполнен мусором, то лишний ноль нужен, чтобы этот мусор не учитывать. Поэтому если мы не знаем чему изначально равно umask, потому что umask(0) в программе нет, будут проблемы с доступом.
